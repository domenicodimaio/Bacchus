...
/**
 * 🔧 FIX CRITICO: Sincronizza lo stato premium con il server
 * Salva lo stato premium nel database per persistenza tra riavvii
 */
export const syncPremiumStatusWithServer = async (isPremium: boolean): Promise<boolean> => {
  try {
    console.log('🔥 SYNC_PREMIUM: Sincronizzazione stato premium con server...');
    
    // Verifica se l'utente è autenticato
    const currentUser = await authService.getCurrentUser();
    if (!currentUser) {
      console.log('⚠️ SYNC_PREMIUM: Utente non autenticato - salvataggio solo locale');
      return false;
    }
    
    // Salva lo stato premium nel database Supabase
    ...
    if (error) {
      console.error('❌ SYNC_PREMIUM: Errore salvataggio server:', error);
      return false;
    }
    
    console.log('✅ SYNC_PREMIUM: Stato premium sincronizzato con server');
    return true;
  } catch (error) {
    console.error('❌ SYNC_PREMIUM: Errore sincronizzazione:', error);
    return false;
  }
};

/**
 * 🔧 FIX CRITICO: Carica lo stato premium dal server
 * Recupera lo stato premium dal database all'avvio dell'app
 */
export const loadPremiumStatusFromServer = async (): Promise<boolean | null> => {
  try {
    console.log('🔄 LOAD_PREMIUM: Caricamento stato premium dal server...');
    
    // Verifica se l'utente è autenticato
    const currentUser = await authService.getCurrentUser();
    if (!currentUser) {
      console.log('⚠️ LOAD_PREMIUM: Utente non autenticato');
      return null;
    }
    
    // Carica lo stato premium dal database
    const { data, error } = await supabase
      .from('user_preferences')
    ...
    if (error) {
      console.log('⚠️ LOAD_PREMIUM: Nessun stato premium trovato nel server:', error.message);
      return null;
    }
    
    const serverPremiumStatus = data?.premium_status || false;
    console.log('✅ LOAD_PREMIUM: Stato premium dal server:', serverPremiumStatus);
    
    return serverPremiumStatus;
  } catch (error) {
    console.error('❌ LOAD_PREMIUM: Errore caricamento:', error);
    return null;
  }
}; 